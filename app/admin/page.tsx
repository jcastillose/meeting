...
export default function Admin(){
  ...
  return <main className="shell admin-shell">
    <header className="topbar"><a className="brand" href="/" style={{color:'inherit',textDecoration:'none'}}><img className="brand-logo" src="/encuentro-logo.png" alt=""/>meeting</a><div className="row"><a href="/">Volver al calendario</a>{account&&<Button variant="outline" disabled={busy} onClick={logout}>Cerrar sesión</Button>}</div></header>
    ...
    {checking?<p role="status">Comprobando sesión…</p>:!account?<section className="panel admin-login"><form className="dialogform" onSubmit={authenticate}>
      ...
      <section className="panel admin-history">
        ...
        {!historyLoading&&entries.map(({poll,responseCount})=><article className="admin-entry" key={poll.id}><div><h2><a href={pollPath(poll)}>{poll.title}</a></h2><p className="muted small">{modes[poll.mode]}{poll.mode==='dates'?` · ${poll.start} al ${poll.end}`:''}</p><p className="muted small">Creada el {new Date(poll.created).toLocaleDateString('es-CL')} · {responseCount} respuestas</p></div><div className="row"><a href={pollPath(poll)}>Abrir</a><Button variant="outline" onClick={async()=>{try{await navigator.clipboard.writeText(new URL(pollPath(poll),location.origin).href);setNotice('Enlace de la consulta copiado.');}catch{setError('No se pudo copiar. Abre la consulta y copia la dirección.');}}}>Copiar enlace</Button></div></article>)}
